...
xode <- function(time, states, parameters){
  with(as.list(c(parameters,states)),{
    ...
    # DEFINE PARAM THAT CHANGE DURING THE RUN (depending on flag)
    #--------------------------------------
    if (flag == 0) {
      aC <- c(aC[1],aC[2],aC3)
      gamma <- 1/tis1
    ...
    list(c(dS0,dE0,dI0,dY0,dR0,dS1,dE1,dI1,dY1,dR1,dS2,dE2,dI2,dY2,dR2,dflag),c(inc=inc,sym=sym,vac1=vac1,vac2=vac2,pImm=pImm))
})}
