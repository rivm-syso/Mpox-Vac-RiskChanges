...
# Same as basic file modelODEvac.r but WITHOUT transitions between risk groups
##########################################################
...
xode <- function(time, states, parameters){
  with(as.list(c(parameters,states)),{
    ...
    #total size of groups with vac status 0,1,2
    #-----------------------------------------------------
    N0 <- S0+E0+I0+Y0+R0
    N1 <- S1+E1+I1+Y1+R1
    N2 <- S2+E2+I2+Y2+R2
    ...
    #mixing matrix: m(i,j)=E*d(i,j)+(1-E)*a(j)*N(j)/Sum[a(k)*N(k)]      
    #---------------------------------------------------
    ...
    #---------------------------------------------------
    foiS <- matrix(nrow=ag,ncol=1) # foi steady partn
    ...
    vac1 <- matrix(nrow=ag,ncol=1)  # vac 1st doses in each sex.act.group (rows) 
    vac2 <- matrix(nrow=ag,ncol=1)  # vac 2nd doses in each sex.act.group (rows) 
    Imm  <- matrix(nrow=ag,ncol=1)  # #immune in each sex.act.group 
